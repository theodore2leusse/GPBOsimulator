...
"""
in this file, we will define a class in order to postprocess the `.npz` file generated by the simulations in `simulation.ipynb` using `simGPBO.py`.
We will use this class in the `results.ipynb` notebook to analyze the results of the simulations.
"""
            ...
            for rep in REP_idx:
                for it in range(self.nb_it): 
                    if measured_location:
                        best_x = self.best_pred_x_measured[emg_i, rep, 0, it] # best prediction's electrode_id that already has been queried (int)
                    else:
